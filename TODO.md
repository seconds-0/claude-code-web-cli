...
### P0: Google OAuth Sign-In (2026-01-11)

**Problem solved:** Google sign-in was failing with "Missing required parameter: client_id"

**Root causes found and fixed:**

1. **Google OAuth credentials** - Created OAuth client in Google Cloud Console, added to Clerk Dashboard
2. **Dual Railway services** - Discovered `untethered.computer` and `www.untethered.computer` served by different Railway services (`web-github` vs `web`) with different branch configurations
3. **Branch mismatch** - `web-github` deployed from `main` branch (stale), `web` deployed from `develop` branch (current)
4. **Wrong Clerk credentials** - `web-github` service had test Clerk credentials (`sk_test_`, dev issuer URL) instead of production

**Fixes applied:**

- [x] Created Google OAuth client (`44874166958-3mmn7vkok99ord7rn75h86umcq4761ll.apps.googleusercontent.com`)
- [x] Merged `develop` to `main` to sync both Railway services
- [x] Updated `web-github` service with production Clerk credentials
- [x] Added `/api/config` endpoint to debug Clerk configuration
- [x] Added `ClerkProvider` URL props to prevent Account Portal redirects

**Verification:** `curl -s https://untethered.computer/api/config | jq '.clerk'` returns `configValid: true`
